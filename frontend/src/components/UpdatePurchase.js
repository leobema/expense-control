...
const endpoint = 'http://localhost:8000/api/purchases/'

const UpdatePurchase = ({purchaseId}) => {
    ...
    const update = async (e) => {
      e.preventDefault()
      await axios.put(`${endpoint}${purchaseId}`, {
        name: name, 
        price: price, 
        stock: stock,
        date: date,
    ...
    useEffect(() => {
        const getPricesById = async () => {
            const response = await axios.get(`${endpoint}${purchaseId}`);
            const { name, price, stock, date, description } = response.data;
            setName(name);
            setPrice(price);
            setStock(stock);
            setDate(date);
